...
class Auth {
	...
	function login($username, $password) {
		...
		if ($result['is_true'] == TRUE) {
			...
			$sucursal = $this->CI->user->getSucursal($qr->sucursal);
			$session_data =	array(
	...
	function create_user($page) {
		...
		$this->CI->form_validation->set_rules('first_name', 'First Name', 'required|trim|max_length[25]|xss_clean');

		$this->CI->form_validation->set_rules('last_name', 'Last Name', 'required|trim|max_length[25]|xss_clean|alpha');

		$this->CI->form_validation->set_rules('role', 'Tipo de Usuario', 'required');
		...
		if ($this->CI->form_validation->run() == FALSE) {
			...
			$new_user_array = array(
								...
								'user_level' => $this->CI->input->post('role'),

								'sucursal' => $this->CI->input->post('sucursal'),
			...
			if ($result['is_true'] == TRUE) {
				...
				$data['message'] = '<div class="alert alert-error">'.$result['message'].'</div>';
	...
	function update_user($page) {
		...
		if ($this->CI->form_validation->run() == FALSE) {
			...
			if ($page == 'secure') {
				...
				$data['message'] = '<div class="alert alert-error">'.$result_get_users['message'].'</div>';
			...
			if ($result_update['is_true'] == TRUE) {
				...
				$data['message'] = '<div class="alert alert-error">'.$result_update['message'].'</div>';
	...
	function is_logged_in() {
		...
		if (!isset($is_logged_in) || $is_logged_in != TRUE ) {
			...
			$this->CI->session->set_flashdata('message', '<div class="alert alert-error">Ingresar Datos.</div>');
	...
	function is_admin() {
		...
		if (!isset($user_level) || $user_level != $admin_level) {
			...
			$this->CI->session->set_flashdata('message', '<div class="alert alert-error">You are most definitely not an admin.</div>');
