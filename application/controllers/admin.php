...
class Admin extends CI_Controller {
	...
	function save_almacenaje() {
		...
		$precio_tarima = $this->input->post("precio_por_tarima");
		$tarifa_almacenamiento = $this->input->post("tarifa_de_almacenamiento");
		...
		$for_save = array(
			'cliente'=>$cliente,
			'esquema_cobranza'=>$esquema,
			'precio_tarima'=>$precio_tarima,
			'tarifa_almacenamiento'=>$tarifa_almacenamiento,
		...
		$this->load->model('tarimas_model');
		$this->tarimas_model->saveAlmacenaje($for_save);
	...
	function actualizar_usuario() {
		...
		if (!empty($password)) {
			...
			$this->user->updateUserPass($id,$username,$email,$password,$first_name,$last_name,$role,$color);
		}else{
			if ($cliente>0) {
				$this->user->updateUserClient($id,$username,$email,$first_name,$last_name,$role,$cliente,$color);	
			}
			$this->user->updateUser($id,$username,$email,$first_name,$last_name,$role,$color);
		}
