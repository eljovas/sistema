	...
	function login() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($result['is_true'] == TRUE &&  $result['is_admin'] == TRUE) {
				...
				redirect('admin', 'location');
			}elseif($result['is_true'] == TRUE && $result['is_admin'] == FALSE && $this->session->userdata('user_level')==1) {
				$this->session->set_flashdata('message', '<div class="alert alert-success">'.$result['message'].'</div>');
				redirect('doctores', 'location');
			}elseif($result['is_true'] == TRUE && $result['is_admin'] == FALSE && $this->session->userdata('user_level')==2) {
				$this->session->set_flashdata('message', '<div class="alert alert-success">'.$result['message'].'</div>');
				redirect('cosmetologas', 'location');
			}elseif($result['is_true'] == TRUE && $result['is_admin'] == FALSE && $this->session->userdata('user_level')==3) {
				$this->session->set_flashdata('message', '<div class="alert alert-success">'.$result['message'].'</div>');
				redirect('recepcionistas', 'location');
			}elseif($result['is_true'] == TRUE && $result['is_admin'] == FALSE && $this->session->userdata('user_level')==4) {
				$this->session->set_flashdata('message', '<div class="alert alert-success">'.$result['message'].'</div>');
				redirect('contabilidad', 'location');
