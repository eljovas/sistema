...
class Biopsias_model extends CI_Model {

	function getAllBiopsias($sucursal) {
		//$this->db->limit(10);
		$this->db->select('id, nombre_completo, telefono, tipo_biopsia, status');
		$this->db->where('sucursal', $sucursal);
		$query = $this->db->get('biopsias');
		return $query->result(); 
	}
/*	function getAllClients($sucursal) {
		$this->db->where('sucursal', $sucursal);
		$query = $this->db->get('clients');
		return $query->result(); 
	}*/
	function save($new_biopsia) {

		$query = $this->db->insert('biopsias', $new_biopsia);
	...
	function updateClient($id,$update_cliente) {

			$this->db->where('id', $id);
			$this->db->update('clients', $update_cliente); 
	}	
