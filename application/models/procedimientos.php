...
class Procedimientos extends CI_Model {
	...
	function updateUserClient($id,$username,$email,$first_name,$last_name,$role,$cliente) {
			$data = array(
               'username' => $username,
               'email' => $email,
               'first_name' => $first_name,
	...
	function verify_and_get_user($username, $password) {
		$this->db->select('user_id, username, first_name, last_name, user_level,client_id');
