...
c+")","error")}b()}})}function j(a,b){var d;try{d=qq.parseJson(b),void 0!==d.newUuid&&(n("Server requested UUID change from '"+s[a]+"' to '"+d.newUuid+"'"),s[a]=d.newUuid,c(a,d.newUuid))}catch(e){n("Error when attempting to parse iframe upload response ("+e+")","error"),d={}}return d}function i(a){var a=a+"_"+x,b=qq.toElement('<iframe src="javascript:false;" name="'+a+'" />');b.setAttribute("id",a);b.style.display="none";document.body.appendChild(b);return b}function r(a,b){var c=m.paramsStore.getParams(a),
d=qq.toElement('<form method="'+(m.demoMode?"GET":"POST")+'" enctype="multipart/form-data"></form>'),e=m.endpointStore.getEndpoint(a),f=e;c[m.uuidParamName]=s[a];m.paramsInBody?qq.obj2Inputs(c,d):f=qq.obj2url(c,e);d.setAttribute("action",f);d.setAttribute("target",b.name);d.style.display="none";document.body.appendChild(d);return d}function k(a){delete p[a];delete s[a];delete v[a];m.cors.expected&&(clearTimeout(u[a]),delete u[a],w.stopReceivingMessages(a));if(a=document.getElementById(a+"_"+x))a.setAttribute("src",
"java"+String.fromCharCode(115)+"cript:false;"),qq(a).remove()}var m=a,p=[],s=[],v={},u={},n=d,w=new qq.WindowReceiveMessage({log:n}),y={},x=qq.getUniqueId(),z;return z={add:function(a){a.setAttribute("name",m.inputName);var b=p.push(a)-1;s[b]=qq.getUniqueId();a.parentNode&&qq(a).remove();return b},getName:function(a){if(z.isValid(a))return p[a].value.replace(/.*(\/|\\)/,"");n(a+" is not a valid item ID.","error")},isValid:function(a){return void 0!==p[a]},reset:function(){p=[];s=[];v={};x=qq.getUniqueId()},
expunge:function(a){return k(a)},getUuid:function(a){return s[a]},cancel:function(a){var b=m.onCancel(a,z.getName(a));return qq.isPromise(b)?b.then(function(){k(a)}):!1!==b?(k(a),!0):!1},upload:function(a){var c=p[a],d=z.getName(a),f=i(a),k;if(!c)throw Error("file with passed id was not added, or already uploaded or cancelled");m.onUpload(a,z.getName(a));k=r(a,f);k.appendChild(c);g(f,function(c){n("iframe loaded");if(!c){var h;try{var g=f.contentDocument||f.contentWindow.document,i=g.body.innerHTML;
...
h[a].lastRequestOverhead=c;if(b===0){h[a].lastChunkIdxProgress=0;h[a].initialRequestOverhead=c;h[a].estTotalRequestsSize=i+e*c}else if(h[a].lastChunkIdxProgress!==b){h[a].lastChunkIdxProgress=b;h[a].estTotalRequestsSize=h[a].estTotalRequestsSize+g}l.onProgress(a,m,d,h[a].estTotalRequestsSize)}};l.onUploadChunk(a,m,s(c));k=l.paramsStore.getParams(a);e(a,k,c);h[a].attemptingResume&&(k[l.resume.paramNames.resuming]=!0);k=i(k,d,c.blob,a);r(a,d);t("Sending chunked upload request for item "+a+": bytes "+
(c.start+1)+"-"+c.end+" of "+g);d.send(k)}function p(a){t("Server has ordered chunking effort to be restarted on next attempt for item ID "+a,"error");A&&(n(a),h[a].attemptingResume=!1);h[a].remainingChunkIdxs=[];delete h[a].loaded;delete h[a].estTotalRequestsSize;delete h[a].initialRequestOverhead}function s(a){return{partIndex:a.part,startByte:a.start+1,endByte:a.end,totalParts:a.count}}function v(a,b){return function(){if(4===b.readyState){var d;if(h[a]){t("xhr - server response received for "+
a);t("responseText = "+b.responseText);try{d=qq.parseJson(b.responseText),void 0!==d.newUuid&&(t("Server requested UUID change from '"+h[a].uuid+"' to '"+d.newUuid+"'"),h[a].uuid=d.newUuid,c(a,d.newUuid))}catch(e){t("Error when attempting to parse xhr response text ("+e+")","error"),d={}}if(200!==b.status||!d.success||d.reset)if(d.reset&&p(a),h[a].attemptingResume&&d.reset)h[a].attemptingResume=!1,t("Server has declared that it cannot handle resume for item ID "+a+" - starting from the first chunk",
"error"),p(a),o.upload(a,!0);else{var i=o.getName(a);l.onAutoRetry(a,i,d,b)||k(a,d,b)}else if(D){var i=h[a].remainingChunkIdxs.shift(),g=f(a,i);h[a].attemptingResume=!1;var i=h[a],j=i.loaded,g=g.size,s;s=B?h[a].lastRequestOverhead:0;i.loaded=j+(g+s);0<h[a].remainingChunkIdxs.length?m(a):(A&&n(a),k(a,d,b))}else k(a,d,b)}}}}function u(a,b){var c=o.getUuid(a),d=h[a].loaded,e=h[a].initialRequestOverhead,f=h[a].estTotalRequestsSize,i=y(a);qq.setCookie(i,c+q+b.part+q+d+q+e+q+f,l.resume.cookiesExpireIn)}
function n(a){h[a].file&&(a=y(a),qq.deleteCookie(a))}function w(a){var b=qq.getCookie(y(a)),a=o.getName(a),c,d,e;if(b){b=b.split(q);if(5===b.length)return a=b[0],c=parseInt(b[1],10),d=parseInt(b[2],10),e=parseInt(b[3],10),b=parseInt(b[4],10),{uuid:a,part:c,lastByteSent:d,initialRequestOverhead:e,estTotalRequestsSize:b};t("Ignoring previously stored resume/chunk cookie for "+a+" - old cookie format","warn")}}function y(a){var b=o.getName(a),a=o.getSize(a),c=l.chunking.partSize,b="qqfilechunk"+q+encodeURIComponent(b)+
q+a+q+c;void 0!==C&&(b+=q+C);return b}function x(a,b){var c;for(c=g(a)-1;c>=b;c-=1)h[a].remainingChunkIdxs.unshift(c);m(a)}function z(a,b,c,d){c=d.part;h[a].loaded=d.lastByteSent;h[a].estTotalRequestsSize=d.estTotalRequestsSize;h[a].initialRequestOverhead=d.initialRequestOverhead;h[a].attemptingResume=!0;t("Resuming "+b+" at partition index "+c);x(a,c)}function F(a,b,c){var d=o.getName(a),e=f(a,b.part),e=l.onResume(a,d,s(e));qq.isPromise(e)?(t("Waiting for onResume promise to be fulfilled for "+a),
e.then(function(){z(a,d,c,b)},function(){t("onResume promise fulfilled - failure indicated.  Will not resume.");x(a,c)})):!1!==e?z(a,d,c,b):(t("onResume callback returned false.  Will not resume."),x(a,c))}function G(a){var b=h[a].file||h[a].blobData.blob,c=o.getName(a),d,e;h[a].loaded=0;d=j(a);d.upload.onprogress=function(b){b.lengthComputable&&(h[a].loaded=b.loaded,l.onProgress(a,c,b.loaded,b.total))};d.onreadystatechange=v(a,d);e=l.paramsStore.getParams(a);b=i(e,d,b,a);r(a,d);t("Sending upload request for "+
a);d.send(b)}function E(a){var b=h[a].xhr;b&&(b.onreadystatechange=null,b.abort());A&&n(a);delete h[a]}var l=a,H=b,t=d,h=[],q="|",D=l.chunking.enabled&&qq.supportedFeatures.chunking,A=l.resume.enabled&&D&&qq.supportedFeatures.resume,C;C=null!==l.resume.id&&void 0!==l.resume.id&&!qq.isFunction(l.resume.id)&&!qq.isObject(l.resume.id)?l.resume.id:void 0;var B=l.forceMultipart||l.paramsInBody,o;return o={add:function(a){var b,c=qq.getUniqueId();if(qq.isFile(a))a=h.push({file:a})-1;else if(qq.isBlob(a.blob))a=
h.push({blobData:a})-1;else throw Error("Passed obj in not a File or BlobData (in qq.UploadHandlerXhr)");if(A&&(b=w(a)))c=b.uuid;h[a].uuid=c;return a},getName:function(a){if(o.isValid(a)){var b=h[a].file,a=h[a].blobData;return b?null!==b.fileName&&void 0!==b.fileName?b.fileName:b.name:a.name}t(a+" is not a valid item ID.","error")},getSize:function(a){a=h[a].file||h[a].blobData.blob;return qq.isFileOrInput(a)?null!=a.fileSize?a.fileSize:a.size:a.size},getFile:function(a){if(h[a])return h[a].file||
...
c){c instanceof a?d[b]=c[0]:a.isPlainObject(c)?(d[b]={},e(c,d[b])):a.isArray(c)?(f=[],a.each(c,function(b,c){c instanceof a?a.merge(f,c):f.push(c)}),d[b]=f):d[b]=c});if(void 0===c)return d}function f(a){var b=[],d=Array.prototype.slice.call(arguments,1);e(d,b);return c()[a].apply(c(),b)}var g="fineUploaderDnd",j;a.fn.fineUploaderDnd=function(d){var e=this,g=arguments,m=[];this.each(function(p,s){j=a(s);if(c()&&"string"===a.type(d)&&"dispose"===d&&void 0!==c()[d]){if(m.push(f.apply(e,g)),1===e.length)return!1}else"object"===
